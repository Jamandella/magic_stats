...
"""class What(Base):
    __tablename__="what"
    huh=mapped_column(SmallInteger,primary_key=True)
class TheFuck(Base):
    __tablename__="theFuck"
    why=mapped_column(SmallInteger,primary_key=True)
    que=mapped_column(SmallInteger,ForeignKey('what.huh'))"""
#the above worked. I don't get what is different.
class ArchRank(Base):
...
def populateCardGameTableV2(conn): #maybe slightly faster. It's small enough to be uncertain.
    #next idea to speed up: process game_data in chunks e.g. take 50K(?) consecutive lines at a time. 
    #if that works, also should apply to archgametable
    cg_table_name=set_abbr+'CardGameStats'
    cg_table=Base.metadata.tables[cg_table_name]
    d=delete(cg_table)
    conn.execute(d)
